...
public partial class AdventureWorksLTContext : DbContext
{
    ...
    public async Task<decimal> ProductCategoryItemsListPriceAverageAsync(int id)
    {
        using (AdventureWorksLTContext db = new AdventureWorksLTContext())
        {
            var result = await db.ProductCategories
                .Include(
                    product => product.Products
                    .Where(product => product.ProductCategoryId == id))
                    .ToListAsync();
            decimal avgPrice = 0;
            int count = 0;
            foreach (var item in result)
            {
                foreach(var price in item.Products)
                {
                    avgPrice += (decimal)price.ListPrice;
                    count ++;
                }              
            }
    ...
    public async Task<IEnumerable<Address>> CustomerAddressesAsync(string CompanyName)
    {
        using (AdventureWorksLTContext db = new AdventureWorksLTContext())
        {
            var result = db.CustomerAddresses
                         .Include(c => c.Customer)
                         .Where(e => e.Customer.CompanyName==CompanyName)
                         .Include(c => c.Address);

            var temp = await result.ToListAsync();
            IEnumerable<Address> addresses = temp.Select(c => c.Address);
    ...
    public async Task<List<string>> ProductCategoryListAsync()
    {
        using (AdventureWorksLTContext db = new AdventureWorksLTContext())
        {
            ...
            var temp = await result
                             .Where(e=> e.ParentProductCategory==null)
                             .ToListAsync();
            ...
            foreach (var item in temp)
            {
                ...
                foreach (var product in item.InverseParentProductCategory)
                {
    ...
    public async Task<Models.CustomerInfo?> GetCustomerInfoAsync(int id, string addressT)
    {
        using (AdventureWorksLTContext db = new AdventureWorksLTContext())
        {
            Models.CustomerInfo? customerInfo = null;
            ...
            var addressType = db.Customers
                                .Where(e => e.CustomerId == id)
                                .Include(e => e.CustomerAddresses
                                .Where(e=>e.AddressType==addressT));

            var addressData = await addressType.ToArrayAsync();
            var cAdd = addressData.Select(e => e.CustomerAddresses.Select(e => e.AddressType));
            if (cAdd.First().IsNullOrEmpty())
            {
            ...
            {
                var finalData = await db.CustomerAddresses
                                .Include(e => e.Customer)
                                .Where(e => e.Customer.CustomerId==id)
                                .Include(e => e.Address)
                                .ToArrayAsync();    
                ...
                {
                    customerInfo.CustomerId = item.CustomerId;
                    customerInfo.Firstname = item.Customer.FirstName;
                    customerInfo.Lastname = item.Customer.LastName;
                    customerInfo.AddressLine1 = item.Address.AddressLine1;
                    customerInfo.City = item.Address.City;  
                }
                return customerInfo;
            }
        }
    ...
    private string GetConnectionString()
    {
        ...
        secretProvider.TryGet("ConnectionStrings:AdventureWorksLT", out var conn);
